...
export function CreateTransferModal({ onClose, onSuccess }: { onClose: () => void, onSuccess: () => void }) {
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-5xl h-[85vh] flex flex-col p-0 overflow-hidden bg-slate-50">
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:grid lg:grid-cols-3 gap-0">
          
          {/* LEFT: PRODUCTS LIST */}
          <div className="lg:col-span-2 flex flex-col h-full bg-white border-r">
            <div className="p-4 border-b space-y-3 flex-shrink-0 bg-slate-50">
              <select 
              ...
              {filteredInventory.length === 0 ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                  {filteredInventory.map((inv: any) => {
                    ...
                    return (
                      <Card key={p.id} className={`cursor-pointer hover:border-blue-400 bg-white shadow-sm ${outOfStock ? 'opacity-50' : ''}`}
                        onClick={() => {
          ...
          <div className="flex flex-col h-full bg-white">
            <CardHeader className="py-4 border-b bg-slate-50 flex-shrink-0"><CardTitle className="text-lg">Transfer Cart ({items.length})</CardTitle></CardHeader>
            <CardContent className="flex-1 flex flex-col p-4 space-y-4 overflow-hidden">
              <div className="flex-1 overflow-y-auto space-y-2 pr-2">
                {items.length === 0 ? (
                  ...
                  <div key={item.cartId} className="flex items-center gap-2 p-2 bg-slate-50 border rounded-md text-sm shadow-sm">
                    <div className="flex-1 leading-tight font-medium text-xs">{item.name}</div>
                    <div className="flex items-center gap-1 bg-white rounded-md border p-0.5 shrink-0">
              ...
              <div className="pt-4 border-t space-y-3 flex-shrink-0 bg-white">
                <Input placeholder="Optional transfer notes..." value={notes} onChange={e => setNotes(e.target.value)} className="bg-slate-50" />
                <Button className="w-full h-12 text-base font-bold bg-blue-600 hover:bg-blue-700" onClick={handleSubmit} disabled={items.length === 0 || !toBranchId || submitMutation.isPending}>
