      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {filteredInventory?.map((inv: any) => {
            ...
            // If it is LPG, render TWO cards side by side
            if (isLpg) {
              return (
                <>
                  {/* Refill Card */}
                  <Card key={`${p.id}-refill`} className="cursor-pointer border-blue-200 hover:border-blue-500 bg-blue-50/30 transition-all" onClick={() => addItem({ ...p, name: `${p.name} (Refill)` }, 1)}>
                  ...
                  {/* Empty Shell Card */}
                  <Card key={`${p.id}-empty`} className="cursor-pointer border-amber-200 hover:border-amber-500 bg-amber-50/30 transition-all" onClick={() => addItem({ ...p, id: p.id + '-empty', name: `${p.name} (Empty Shell)`, price: 3500 }, 1)}>
                    <CardContent className="p-4 flex flex-col h-full justify-between">
                      <div>
...
export default NewSale    } else if (type === 'EMPTY') {
